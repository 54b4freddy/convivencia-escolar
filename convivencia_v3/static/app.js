...
function buildNav(){
  ...
  (NAV[CU.rol]||[]).forEach(item=>{
    if(item.sep){const d=document.createElement('div');d.className='ni-sep px-0 pb-1.5 pt-4 text-[11px] font-semibold uppercase tracking-wide text-white/30';d.textContent=item.sep;nav.appendChild(d);return;}
    const d=document.createElement('div');d.className='ni flex cursor-pointer items-center gap-2.5 rounded-lg border-l-[3px] border-transparent px-3 py-2.5 text-[13px] font-medium leading-snug text-white/50 transition-all hover:bg-white/[0.06] hover:text-white/90';d.textContent=item.l;d.setAttribute('data-tab',item.id);
